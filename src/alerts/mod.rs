...
use crate::monitoring::{AlertLevel, SystemAlert};
...
impl AlertManager {
    ...
    pub async fn critical_error(
        ...
        let alert = SystemAlert::new(AlertLevel::Critical, title, description, component);
    ...
    pub async fn high_priority(
        ...
        let alert = SystemAlert::new(AlertLevel::High, title, description, component);

        self.send_alert(&alert).await
    }

    /// Tworzy alert o średnim priorytecie
    pub async fn medium_priority(
        &mut self,
        title: String,
        description: String,
        component: String,
    ) -> Result<()> {
        let alert = SystemAlert::new(AlertLevel::Medium, title, description, component);
...
impl RateLimiter {
    ...
    pub fn cleanup(&mut self) {
        ...
        self.last_sent
            .retain(|_, &mut last_sent| last_sent > cutoff);
    }
}
