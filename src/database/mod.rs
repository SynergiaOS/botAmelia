...
impl DatabaseManager {
        ...
        F: std::future::Future<Output = Result<T>>,
    {
        ...
        let span =
            sentry::start_transaction(sentry::TransactionContext::new("database", operation_name));
    ...
    pub async fn create_backup(&self) -> Result<String> {
        ...
        tokio::fs::copy(&self.config.path, &backup_path)
            .await
    ...
    pub async fn cleanup_old_backups(&self, keep_count: usize) -> Result<()> {
        ...
        let mut entries = tokio::fs::read_dir(&self.config.backup_directory)
            .await
