...
fn test_security_event_type_equality() {
    assert_eq!(SecurityEventType::UnauthorizedAccess, SecurityEventType::UnauthorizedAccess);
    assert_ne!(SecurityEventType::UnauthorizedAccess, SecurityEventType::SuspiciousTransaction);
}
...
fn test_security_monitor_convenience_methods() {
    ...
    monitor.unauthorized_access("Unauthorized API access".to_string(), Some("192.168.1.1".to_string()));
    
    // Test suspicious transaction
    monitor.suspicious_transaction("Large transaction".to_string(), json!({"amount": 10000}));
    
    // Test trading anomaly
    monitor.trading_anomaly("Unusual trading pattern".to_string(), json!({"pattern": "high_frequency"}));
...
fn test_security_validator_ip_allowlist() {
    ...
    assert!(SecurityValidator::is_ip_allowed("192.168.1.100", &allowed_ranges));
    assert!(SecurityValidator::is_ip_allowed("10.0.0.50", &allowed_ranges));
    assert!(SecurityValidator::is_ip_allowed("127.0.0.1", &allowed_ranges));
    
    assert!(!SecurityValidator::is_ip_allowed("172.16.0.1", &allowed_ranges));
    assert!(!SecurityValidator::is_ip_allowed("8.8.8.8", &allowed_ranges));
}

#[test]
fn test_security_validator_auth_token() {
    // Valid tokens
    assert!(SecurityValidator::validate_auth_token("abcdef1234567890abcdef1234567890abcdef12"));
    assert!(SecurityValidator::validate_auth_token("1234567890abcdef1234567890abcdef12345678"));
    
    // Invalid tokens
    assert!(!SecurityValidator::validate_auth_token("")); // Empty
    assert!(!SecurityValidator::validate_auth_token("short")); // Too short
    assert!(!SecurityValidator::validate_auth_token("invalid-chars-!@#$%^&*()")); // Invalid characters
    assert!(!SecurityValidator::validate_auth_token("spaces in token are not allowed"));
}

#[test]
fn test_security_validator_password_strength() {
    // Weak passwords
    assert_eq!(SecurityValidator::check_password_strength(""), PasswordStrength::Weak);
    assert_eq!(SecurityValidator::check_password_strength("123"), PasswordStrength::Weak);
    assert_eq!(SecurityValidator::check_password_strength("password"), PasswordStrength::Weak);
    
    // Medium passwords
    assert_eq!(SecurityValidator::check_password_strength("Password1"), PasswordStrength::Medium);
    assert_eq!(SecurityValidator::check_password_strength("mypassword123"), PasswordStrength::Medium);
    
    // Strong passwords
    assert_eq!(SecurityValidator::check_password_strength("MyPassword123"), PasswordStrength::Strong);
    assert_eq!(SecurityValidator::check_password_strength("SecurePass2024"), PasswordStrength::Strong);
    
    // Very strong passwords (need score > 6)
    assert_eq!(SecurityValidator::check_password_strength("MySecure!Password123"), PasswordStrength::Strong);
    assert_eq!(SecurityValidator::check_password_strength("Tr@d1ng$ystem2024!"), PasswordStrength::Strong);
}
...
fn test_security_edge_cases() {
    ...
    assert!(!SecurityValidator::is_ip_allowed("192.168.1.1", &empty_ranges));
    ...
    assert_eq!(SecurityValidator::check_password_strength("12345678"), PasswordStrength::Weak);
    assert_eq!(SecurityValidator::check_password_strength("ABCDEFGH"), PasswordStrength::Weak);
    assert_eq!(SecurityValidator::check_password_strength("abcdefgh"), PasswordStrength::Weak);
}
