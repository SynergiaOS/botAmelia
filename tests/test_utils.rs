...
use cerberus::{
    config::Config,
    signals::{Signal, Confidence, SignalSource},
    risk::{Portfolio, Position, TradeSide},
    trading::{TradeOrder, ExecutionResult},
    cache::CacheManager,
    ...
    pub fn print_summary(&self) {
        ...
        for (name, measurements) in &self.measurements {
            if let Some(avg) = self.get_average(name) {
                ...
                println!("{}: {} samples, avg: {:?}, p50: {:?}, p95: {:?}",
                         name, measurements.len(), avg, p50, p95);
            }
        }
    }
}
...
impl TestAssertions {
    ...
    pub fn assert_within_percent(actual: f64, expected: f64, percent: f64) {
        ...
        assert!(
            ...
            actual, percent, expected, diff, tolerance
        );
    }
...
mod tests {
    ...
    fn test_assertions() {
        ...
        TestAssertions::assert_duration_within(Duration::from_millis(50), Duration::from_millis(100));
