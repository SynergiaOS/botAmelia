...
fn test_signal_creation_performance() {
    ...
    println!("Signal creation: {} signals in {}ms (avg: {}ms per signal)", 
             1000, duration.as_millis(), avg_time_ms);
    
    // Each signal creation should be very fast
    assert!(avg_time_ms < 1, "Signal creation too slow: {}ms average", avg_time_ms);
}

#[test]
fn test_signal_validation_performance() {
    // Create test signals
    let signals: Vec<Signal> = (0..1000).map(|i| {
        Signal::new(
            format!("TOKEN_{}", i),
            "performance_test".to_string(),
            Confidence::Medium,
    ...
    println!("Signal validation: {} validations in {}ms (avg: {}ms per validation)", 
             1000, duration.as_millis(), avg_time_ms);
    
    assert!(avg_time_ms < 1, "Signal validation too slow: {}ms average", avg_time_ms);
}
...
fn test_signal_hash_calculation_performance() {
    ...
    println!("Hash calculation: 10000 hashes in {}ms (avg: {}ns per hash)", 
             duration.as_millis(), avg_time_ns);
    
    // Hash calculation should be very fast (< 1ms for 10k operations)
    assert!(duration.as_millis() < 100, "Hash calculation too slow: {}ms for 10k operations", duration.as_millis());
}
...
fn test_portfolio_update_performance() {
    ...
    println!("Portfolio update: 1000 updates with 100 positions in {}ms (avg: {}ms per update)", 
             duration.as_millis(), avg_time_ms);
    
    assert!(avg_time_ms < PORTFOLIO_UPDATE_MAX_MS, 
            "Portfolio update too slow: {}ms average (max: {}ms)", avg_time_ms, PORTFOLIO_UPDATE_MAX_MS);
}

#[test]
fn test_position_pnl_calculation_performance() {
    let mut positions: Vec<Position> = (0..1000).map(|i| {
        Position::new(
            format!("TOKEN_{}", i),
            if i % 2 == 0 { TradeSide::Long } else { TradeSide::Short },
            100.0,
    ...
    println!("PnL calculation: 1000 calculations in {}ms (avg: {}ns per calculation)", 
             duration.as_millis(), avg_time_ns);
    
    // PnL calculation should be very fast
    assert!(duration.as_millis() < 10, "PnL calculation too slow: {}ms for 1000 calculations", duration.as_millis());
}
...
fn test_trade_order_creation_performance() {
    ...
    println!("Trade order creation: 1000 orders in {}ms (avg: {}ms per order)", 
             duration.as_millis(), avg_time_ms);
    
    assert!(avg_time_ms < 1, "Trade order creation too slow: {}ms average", avg_time_ms);
}

#[test]
fn test_trade_order_validation_performance() {
    let orders: Vec<TradeOrder> = (0..1000).map(|i| {
    ...
    println!("Trade order validation: 1000 validations in {}ms (avg: {}ms per validation)", 
             duration.as_millis(), avg_time_ms);
    
    assert!(avg_time_ms < 1, "Trade order validation too slow: {}ms average", avg_time_ms);
}
...
fn test_cache_entry_operations_performance() {
    ...
    println!("Cache entry creation: 10000 entries in {}ms", creation_time.as_millis());
    println!("Cache entry access: 10000 operations in {}ms", access_time.as_millis());
    
    assert!(creation_time.as_millis() < 100, "Cache entry creation too slow: {}ms", creation_time.as_millis());
    assert!(access_time.as_millis() < 50, "Cache entry access too slow: {}ms", access_time.as_millis());
}
...
fn test_cache_stats_update_performance() {
    ...
    println!("Cache stats updates: 20000 operations in {}ms", duration.as_millis());
    
    assert!(duration.as_millis() < 50, "Cache stats updates too slow: {}ms", duration.as_millis());
}
...
fn test_liquidation_price_calculation_performance() {
    ...
    for i in 0..10000 {
        ...
        let _long_liq = Position::calculate_liquidation_price(&TradeSide::Long, price, leverage as u8);
        let _short_liq = Position::calculate_liquidation_price(&TradeSide::Short, price, leverage as u8);
    }
    
    let duration = start.elapsed();
    let avg_time_ns = duration.as_nanos() / 20000; // 2 calculations per iteration
    
    println!("Liquidation price calculation: 20000 calculations in {}ms (avg: {}ns per calculation)", 
             duration.as_millis(), avg_time_ns);
    
    assert!(duration.as_millis() < 10, "Liquidation price calculation too slow: {}ms", duration.as_millis());
}
...
fn stress_test_concurrent_signal_processing() {
    ...
    for thread_id in 0..4 {
        ...
        let handle = thread::spawn(move || {
            for i in 0..250 { // 250 signals per thread = 1000 total
                let signal = Signal::new(
    ...
    println!("Stress test: {} signals processed in {}ms across 4 threads", 
             processed, duration.as_millis());
    
    assert_eq!(processed, 1000);
    assert!(duration.as_millis() < 1000, "Stress test too slow: {}ms", duration.as_millis());
}
...
fn memory_usage_test() {
    ...
    println!("Created {} signals, {} portfolios, {} positions", 
             signals.len(), portfolios.len(), positions.len());
    ...
    println!("Random access test completed in {}ms", access_time.as_millis());
    assert!(access_time.as_millis() < 10, "Random access too slow: {}ms", access_time.as_millis());
}
...
fn test_serialization_performance() -> Result<()> {
    ...
    println!("Serialization: 2000 objects in {}ms", serialization_time.as_millis());
    println!("Deserialization: 2000 objects in {}ms", deserialization_time.as_millis());
    
    assert!(serialization_time.as_millis() < 100, "Serialization too slow: {}ms", serialization_time.as_millis());
    assert!(deserialization_time.as_millis() < 100, "Deserialization too slow: {}ms", deserialization_time.as_millis());
...
fn benchmark_critical_path_operations() {
    ...
    println!("Critical path benchmark: 100 complete cycles in {}ms (avg: {}ms per cycle)", 
             duration.as_millis(), avg_time_ms);
    
    // Critical path should be very fast (< 10ms per complete cycle)
    assert!(avg_time_ms < SIGNAL_PROCESSING_MAX_MS, 
            "Critical path too slow: {}ms average (max: {}ms)", avg_time_ms, SIGNAL_PROCESSING_MAX_MS);
}
