...
fn test_health_report_creation() {
    let report = HealthReport::new(HealthStatus::Healthy, "All systems operational".to_string());
...
fn test_health_report_with_checks() {
    let mut report = HealthReport::new(HealthStatus::Warning, "Some issues detected".to_string());
...
fn test_health_check_creation() {
    let check = HealthCheck::new(HealthStatus::Critical, "Service unavailable".to_string());
...
fn test_health_check_with_metadata() {
    ...
    let check = HealthCheck::new(HealthStatus::Warning, "Performance degraded".to_string())
...
fn test_monitoring_serialization() -> Result<()> {
    let metrics = SystemMetrics::new();
    let health_check = HealthCheck::new(HealthStatus::Healthy, "All good".to_string());
    let alert = Alert::new(
        AlertLevel::Info,
        "Test".to_string(),
        "Test message".to_string(),
