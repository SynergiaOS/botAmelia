...
impl TestResults {
    ...
    pub fn print_summary(&self) {
        ...
        println!("Unit Tests:        {} passed, {} failed, {} skipped ({}ms)", 
                 self.unit_tests.passed, self.unit_tests.failed, self.unit_tests.skipped, self.unit_tests.duration_ms);
        println!("Integration Tests: {} passed, {} failed, {} skipped ({}ms)", 
                 self.integration_tests.passed, self.integration_tests.failed, self.integration_tests.skipped, self.integration_tests.duration_ms);
        println!("Performance Tests: {} passed, {} failed, {} skipped ({}ms)", 
                 self.performance_tests.passed, self.performance_tests.failed, self.performance_tests.skipped, self.performance_tests.duration_ms);
        println!("Security Tests:    {} passed, {} failed, {} skipped ({}ms)", 
                 self.security_tests.passed, self.security_tests.failed, self.security_tests.skipped, self.security_tests.duration_ms);
...
macro_rules! run_test_suite {
    ($suite_name:expr, $test_fn:expr) => {{
        ...
        match result {
            Ok(Ok(())) => {
                println!("✅ {} tests completed successfully in {}ms", $suite_name, duration.as_millis());
                Ok(())
            }
            Ok(Err(e)) => {
...
mod tests {
    ...
    fn test_category_from_str() {
        ...
        assert_eq!(TestCategory::from_str("INTEGRATION"), Some(TestCategory::Integration));
        assert_eq!(TestCategory::from_str("performance"), Some(TestCategory::Performance));
