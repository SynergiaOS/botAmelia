...
use cerberus::{
    ...
    signals::{Signal, Confidence, SignalStats},
    risk::{Portfolio, Position, TradeSide},
    trading::{TradeOrder, OrderType, ExecutionResult, TradingStats},
    cache::{CacheManager, CacheStats},
    security::{SecurityMonitor, SecurityEventType, SecurityLevel},
    monitoring::{SystemMetrics, HealthReport, HealthStatus},
};
...
async fn test_performance_requirements() -> Result<()> {
    ...
    assert!(duration.as_millis() < 10, "Signal processing too slow: {}ms", duration.as_millis());
    ...
    assert!(duration.as_millis() < 100, "Portfolio operations too slow: {}ms", duration.as_millis());
...
async fn stress_test_high_frequency_trading() -> Result<()> {
    ...
    for i in 0..1000 {
        let signal = Signal::new(
            ...
            if i % 4 == 0 { Confidence::High } else { Confidence::Medium },
...
async fn test_security_monitoring_integration() -> Result<()> {
    ...
    let unauthorized_events = security_monitor.events_by_type(SecurityEventType::UnauthorizedAccess);
